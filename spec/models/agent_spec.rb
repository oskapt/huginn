...
describe AgentDrop do
  ...
  it 'should have .url' do
    t = '{{ agent.url }}'
    expect(interpolate(t, @wsa1)).to match(/http:\/\/#{ENV['DOMAIN']}(?::\d+)?\/agents\/#{@wsa1.id}/)
    expect(interpolate(t, @wsa2)).to match(/http:\/\/#{ENV['DOMAIN']}(?::\d+)?\/agents\/#{@wsa2.id}/)
    expect(interpolate(t, @efa)).to  match(/http:\/\/#{ENV['DOMAIN']}(?::\d+)?\/agents\/#{@efa.id}/)
  end
end
